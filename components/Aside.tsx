...
export default function Aside({ mainContent, onNavigate, children }: AsideProps & { children?: ReactNode }) {
  ...
  return (
    <AsideProvider navigateToPage={navigateToPage} resetToMain={resetToMain}>
      <AsideInner mainContent={mainContent} pages={pages} setPages={setPages} currentIndex={currentIndex} setCurrentIndex={setCurrentIndex} isAnimating={isAnimating} setIsAnimating={setIsAnimating} goBack={goBack} resetToMain={resetToMain} />
      {children}
    </AsideProvider>
  )
}
    ...
    <aside className='C013'>
      <div className='C089'>
        {pages.map((page, index) => {
          ...
          const isSlidePageComponent = React.isValidElement(page.content) && 
