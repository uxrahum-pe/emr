...
export default function WeeklyCalendar() {
  ...
  const handleDayClick = (weekIndex: number, dayIndex: number, e: React.MouseEvent) => {
    e.stopPropagation()
    const elementIndex = weekIndex * 7 + dayIndex
    const weekDays = generateWeekDays(weekIndex - 25 + weekOffset)
    const day = weekDays[dayIndex]
  ...
  useEffect(() => {
    ...
    setTimeout(centerOnToday, 300) // 한 번 더 시도
  }, [weekOffset])
  ...
  return (
    <div className='C057'>
      <p className='T029'>{centerDate.year}<span className='isUnit'>년</span> {String(centerDate.month).padStart(2, '0')}<span className='isUnit'>월</span></p>
        ...
        <div className='C054'>
          {Array.from({ length: 50 }, (_, weekIndex) => {
            ...
            return weekDays.map((day, dayIndex) => {
              const elementIndex = weekIndex * 7 + dayIndex
              const opacity = isInitialized && opacities[elementIndex] !== undefined ? opacities[elementIndex] : 0
              return (
                <div
                  key={`week-${weekIndex}-day-${dayIndex}`}
                  className={`C055 ${day.isToday ? 'isToday' : ''} ${selectedDayIndex === elementIndex ? 'isSelected' : ''}`}
                  style={{ opacity }}
                >
                  <p className='T027'>{day.dayName}</p>
                  <div className='C056' onClick={(e) => handleDayClick(weekIndex, dayIndex, e)}>
      ...
      <div className='C058'>
        <Popup
          ...
          />}
        >
          <div className='C059' onClick={() => setIsPopupOpen(true)}><p className='T030'><span className='isUnit'>선택 날짜:</span> {selectedDate ? formatDate(selectedDate) : '선택 안됨'}</p><div className='C060 styleSheet isIcon isCalendar'></div></div>
        </Popup>
        <div className='C059 isToday' onClick={handleGoToToday}><p className='T031'>오늘로 이동</p></div>
      </div>
    </div>
  )
}
